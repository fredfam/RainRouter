...
export async function fetchLiveWeatherForecast(): Promise<WeatherForecastItem[]> {
  ...
  try {
    ...
    const res = await fetch('/api/datagov/weather-forecast', {
    ...
    if (res.ok) {
      ...
      const forecasts = parseWeatherForecast(data);
  ...
  try {
    ...
    console.warn('Using Singapore weather fallback data:', err);
  }
  return INITIAL_WEATHER_FORECAST;
}

function parseWeatherForecast(data: any): WeatherForecastItem[] {
  if (data?.data?.items?.[0]?.forecasts && Array.isArray(data.data.items[0].forecasts)) {
    return data.data.items[0].forecasts.slice(0, 10).map((f: { area: string; forecast: string }) => {
...
export async function fetchLivePsi(): Promise<PsiData> {
  ...
  try {
    ...
    const res = await fetch('/api/datagov/psi', {
      signal: controller.signal,
      headers: { Accept: 'application/json' }
    });
    clearTimeout(timeoutId);
...
function parsePsiData(data: any): PsiData | null {
...
export async function fetchLiveTrafficCameras(): Promise<TrafficCamera[]> {
  ...
  try {
    ...
    const res = await fetch('/api/datagov/traffic-images', {
      signal: controller.signal,
      headers: { Accept: 'application/json' }
    });
    clearTimeout(timeoutId);
    ...
    if (res.ok) {
      const data = await res.json();
      const cameras = parseTrafficCameras(data);
...
function parseTrafficCameras(data: any): TrafficCamera[] {
...
export async function fetchLiveCarparks(): Promise<CarparkInfo[]> {
  ...
  try {
    ...
    const res = await fetch('/api/datagov/carpark-availability', {
      signal: controller.signal,
      headers: { Accept: 'application/json' }
    });
    clearTimeout(timeoutId);
    ...
    if (res.ok) {
      const data = await res.json();
      const carparks = parseCarparks(data);
...
function parseCarparks(data: any): CarparkInfo[] {
